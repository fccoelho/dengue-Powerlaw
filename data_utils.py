...
from fitpl import (
    FitPL,
    fetch_infodengue as _fetch_infodengue,
    fetch_episcanner as _fetch_episcanner,
)
...
def get_alpha_trends(db_path="powerlaw_results.db"):
    with sqlite3.connect(db_path) as conn:
        df_yearly = pd.read_sql_query(
            "SELECT geocode, year, alpha FROM powerlaw_fits_yearly", conn
        )
...
def get_yearly_db_data(geocode, db_path="powerlaw_results.db"):
    with sqlite3.connect(db_path) as conn:
        df = pd.read_sql_query(
            "SELECT * FROM powerlaw_fits_yearly WHERE geocode=?",
            conn,
            params=(geocode,),
        )
    return df
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        if state_code:
            ...
            return pd.read_sql_query(
                "SELECT * FROM powerlaw_fits_yearly WHERE geocode >= ? AND geocode < ?",
                conn,
                params=(min_geo, max_geo),
            )

    return pd.DataFrame()


def get_episcanner_fit_results(
    region, metric="total_cases", db_path="powerlaw_results.db"
):
    """Fetch Episcanner fit results for a region (state or 'BR') for all years."""
    with sqlite3.connect(db_path) as conn:
        if region == "BR":
            df = pd.read_sql_query(
                "SELECT * FROM episcanner_fits WHERE metric=? ORDER BY year ASC",
                conn,
                params=(metric,),
            )
        else:
            df = pd.read_sql_query(
                "SELECT * FROM episcanner_state_fits WHERE state=? AND metric=? ORDER BY year ASC",
                conn,
                params=(region, metric),
            )
...
def get_merged_data(state_abbrev=None, include_trends=False):
    ...
    merged = gdf.merge(
        ...
        left_on="code_muni",
        right_on="geocode",
        how="inner" if state_abbrev else "right",
    )
...
def get_city_details(geocode_str):
    ...
    with sqlite3.connect("powerlaw_results.db") as conn:
        df = pd.read_sql_query(
            "SELECT * FROM powerlaw_fits WHERE geocode=?", conn, params=(geocode,)
        )
    return df
...
def get_yearly_details(geocode_str):
    ...
    return df[["year", "alpha", "xmin", "xmax", "R", "p"]].sort_values(
