...
from data_utils import (
    ...
    get_alpha_trends,
    get_yearly_db_data,
    get_yearly_state_data,
    get_episcanner_fit_results,
    ensure_episcanner_files,
    get_duckdb_episcanner_data,
    extract_geocode,
    fetch_infodengue,
    fetch_episcanner,
    get_combined_indicator_data,
)


def _get_centroid_lat_lon(gdf, as_list=False):
    """Calculate centroid lat/lon properly, handling geographic CRS.

    Args:
        gdf: GeoDataFrame
        as_list: If True, returns lists of all centroids. If False, returns mean values.
    """
    if gdf.crs and gdf.crs.is_geographic:
        projected = gdf.to_crs(epsg=3857)
        centroids = projected.geometry.centroid.to_crs(epsg=4326)
        if as_list:
            return list(centroids.y), list(centroids.x)
        return float(centroids.y.mean()), float(centroids.x.mean())
    if as_list:
        return list(gdf.geometry.centroid.y), list(gdf.geometry.centroid.x)
...
def plot_combined_episcanner_fit(region, metric="total_cases"):
    ...
    if fit_df.empty:
        return go.Figure().update_layout(
            title=f"No fitted parameters found in DB for {region} (Combined {metric_label})"
        )
    ...
    if year_0_fit.empty:
        return go.Figure().update_layout(
            title=f"No combined (year=0) fit found for {region}"
        )
...
def plot_episcanner_state_map(metric="total_cases"):
    ...
    metric_label = (
        "Epidemic Size's alpha"
        if metric == "total_cases"
        else "Epidemic Duration's alpha"
    )

    with sqlite3.connect("powerlaw_results.db") as conn:
        df_states = pd.read_sql_query(
            "SELECT state, alpha FROM episcanner_state_fits WHERE year=0 AND metric=?",
            conn,
            params=(metric,),
        )
    ...
    gdf_states = (
        load_geography()[["abbrev_state", "geometry"]]
        .dissolve(by="abbrev_state")
        .reset_index()
    )
    gdf_states = gdf_states.merge(
        df_states, left_on="abbrev_state", right_on="state", how="left"
    )
    ...
    fig.update_layout(
        margin={"r": 0, "t": 40, "l": 0, "b": 0}, width=800, height=600, autosize=True
    )
    return fig
...
def plot_episcanner_region_timeseries(region):
    ...
    try:
        ...
        if region == "BR":
            ...
            if not files:
                return go.Figure().update_layout(
                    title="No state cache files found for Brazil aggregation"
                )
            ...
            if not os.path.exists(file_path):
                ...
                if df is None or df.empty:
                    return go.Figure().update_layout(
                        title=f"No timeseries data found for {region}"
                    )
                combined = df
            else:
...
def plot_state_residuals_vs_alpha(region):
    ...
    if merged.empty:
        return go.Figure().update_layout(
            title=f"No overlapping data (alpha & residuals) for {region}"
        )
...
def plot_map(state_abbrev=None):
    ...
    if state_abbrev is None:
        ...
        centroid_lat, centroid_lon = _get_centroid_lat_lon(merged)
    ...
    if not significant.empty:
        sig_lat, sig_lon = _get_centroid_lat_lon(significant, as_list=True)
        fig.add_trace(
            go.Scattermap(
                lat=sig_lat,
                lon=sig_lon,
...
def plot_trend_map(state_abbrev=None):
    ...
    if (
        merged.empty
        or "alpha_trend" not in merged.columns
        or merged["alpha_trend"].isna().all()
    ):
...
def plot_local_moran(state_abbrev=None):
    if state_abbrev is None:
        return go.Figure().update_layout(
            title="Select a state to see Local Moran clustering"
        )
    ...
    if merged.empty or "alpha" not in merged.columns or merged["alpha"].isna().all():
        return go.Figure().update_layout(
            title="Insufficient data for Local Moran clustering"
        )
    ...
    try:
        ...
        centroid_lat, centroid_lon = _get_centroid_lat_lon(df_clean)
        fig = px.choropleth_map(
            df_clean,
...
def plot_alpha_vs_incidence(state_abbrev=None):
    ...
    if (
        merged.empty
        or "alpha" not in merged.columns
        or "incidence" not in merged.columns
    ):
...
def plot_fit(geocode_str):
    ...
    with sqlite3.connect("powerlaw_results.db") as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT alpha, xmin, xmax FROM powerlaw_fits WHERE geocode=?", (geocode,)
        )
        row = cursor.fetchone()
...
def plot_yearly_trend(geocode_str):
    ...
    if df_yearly.empty:
        return go.Figure().update_layout(
            title=f"No yearly trend data for {geocode_str}"
        )
    ...
    if df.empty:
        return [
            go.Figure().update_layout(
                title="No episcanner data available for correlation"
            )
        ] * 5
